#!/bin/bash
# install-linpac-signalink-d700.sh
# Linpac + Direwolf + SignaLink USB + Kenwood TM-D700 on Raspberry Pi 3B+
# SignaLink uses VOX PTT (DELAY knob). Do NOT write RTS PTT into direwolf.conf.
# Do not run this as a root login. Use a normal user; the script calls sudo itself.
#
# Pi 3B+ has 1 GB RAM. Builds use -j2 to reduce out-of-memory failures.
# Binaries match the OS: armhf (32-bit) or arm64 (64-bit). Do not mix a 64-bit
# kernel with 32-bit userland.
#
# From a PC:
...
#
# One-shot (passwordless sudo):
#   ssh -t USER@PI_ADDRESS 'curl -fsSL https://raw.githubusercontent.com/buryd/raspberry-Pi_Linpac_Direwolf_digirig_Icom4100/main/install-linpac-signalink-d700.sh | bash -s -- --callsign YOURCALL'
#
# CALLSIGN=YOURCALL ./install-linpac-signalink-d700.sh
...
SKIP_LINPAC=0
FORCE_REBUILD=0
# Pi 3B+ default: 2 compile jobs (1 GB RAM). Override with MAKE_JOBS=1 if cmake/make is killed.
...
Install Linpac + Direwolf for SignaLink USB and a Kenwood TM-D700 on Raspberry Pi 3B+.
Safe to run over SSH. Does not start Direwolf or Linpac.

PTT is SignaLink VOX (no /dev/ttyUSB0 RTS). Cable: SLCAB6PM into the D700 DATA jack,
not the control-head DB-9 COM port. Leave the D700 built-in TNC OFF.
...
log "============================================================"
log "Linpac / Direwolf / SignaLink / TM-D700 installer (Pi 3B+)"
log "Time:      $(date -Is)"
log "User:      $REAL_USER"
log "Home:      $REAL_HOME"
log "Callsign:  $MYCALL"
log "AX.25 port:$AX25_PORT"
log "Make jobs: $MAKE_JOBS"
log "Kernel:    $(uname -m) ($(uname -r))"
log "Userland:  $(dpkg --print-architecture 2>/dev/null || echo unknown)"
...
if [[ "$KERN" == "aarch64" && "$USERLAND" == "armhf" ]]; then
  die "64-bit kernel with 32-bit userland. Install Raspberry Pi OS 32-bit or 64-bit consistently, then rerun."
fi
if [[ "$KERN" == "armv7l" && "$USERLAND" == "arm64" ]]; then
  die "32-bit kernel with 64-bit userland. Use a matching Raspberry Pi OS image, then rerun."
fi
...
log "==> Adding $REAL_USER to the audio group (SignaLink is USB audio; stock unit has no RTS serial PTT)"
run_root usermod -aG audio "$REAL_USER"
...
$AX25_PORT   $MYCALL   19200  255     2       2m 1200 SignaLink TM-D700
EOF
)
TMP_AX="$(mktemp)"
if [[ -f "$AXPORTS" ]]; then
...
detect_audio_device() {
  ...
  if command -v aplay >/dev/null; then
    line="$(aplay -l 2>/dev/null | grep -iE 'USB|SignaLink|Device|C-Media|PnP|CODEC' | head -n1 || true)"
...
log "==> Detected ADEVICE=$ADEVICE (SignaLink USB audio)"
log "    Plug in the SignaLink before first start if this looks wrong, then edit $DIREWOLF_CONF"
log "    Stock SignaLink has no serial PTT; direwolf.conf will not contain PTT ... RTS"

write_direwolf_conf() {
  local dest="$1"
  cat >"$dest" <<EOF
# Generated by $SCRIPT_NAME on $(date -Is)
# SignaLink USB + Kenwood TM-D700 on Raspberry Pi 3B+
# PTT is VOX on the SignaLink (DELAY knob). Do not add PTT /dev/ttyUSB0 RTS.
# Confirm audio with: aplay -l
# Radio: TNC OFF, Menu 1-9-6 DATA SPEED 1200, SLCAB6PM into DATA jack (not DB-9 COM)
...
MYCALL $MYCALL
MODEM 1200
# SignaLink USB keys the radio with VOX. Leave PTT commented out.
# PTT /dev/ttyUSB0 RTS

# Slightly longer lead-in/tail so VOX can key and unkey (10 ms units)
DWAIT 0
TXDELAY 40
TXTAIL 20
...
if [[ "$SKIP_DIREWOLF" -eq 0 ]] && need_build_direwolf; then
  ...
  if [[ "$dw_fetch" -eq 0 ]]; then
    as_user_bash "cd '$SRC_DIR/direwolf' && rm -rf build && mkdir build && cd build && cmake .. && make -j$MAKE_JOBS"
...
log ""
log "IMPORTANT — audio group: $REAL_USER was added to audio."
log "  Disconnect SSH and log back in before the first start so group membership applies."
log ""
log "Next (after reconnecting SSH):"
log "  1. Plug SignaLink USB into the Pi. SLCAB6PM into TM-D700 DATA jack (not DB-9 COM)."
log "     Confirm:  aplay -l"
log "     Edit $DIREWOLF_CONF if ADEVICE is wrong. Do not add PTT RTS."
log "  2. TM-D700: built-in TNC OFF, FM simplex, Menu 1-9-6 = 1200."
log "     SignaLink knobs: TX 9-10 o'clock, RX 10-12, DELAY short (8-9 o'clock)."
